...
- **Logo**: `assets/images/logo-icon.png` — a pixel-accurate cutout of the
  real corecompute.ae logo's icon (not a hand-traced approximation), recolored
  solid blue (`#3d8bff`) with a transparent background. Generated by
  lightness-thresholding the original raster (background vs. ink, smoothed
  at the edges) rather than trying to unmatte per-pixel color — that
  threshold approach is robust to JPEG compression noise; an earlier
  per-pixel alpha/color attempt produced blotchy fringing artifacts and was
  discarded. `logo-mark.png` is the same technique applied to the *full*
  lockup (icon + "CORE COMPUTE" wordmark), two-toned blue/amber, used only
  for the JSON-LD `logo` field and social-share metadata — the nav/footer
  use `logo-icon.png` (icon only) paired with real HTML text
  (`.brand-name`), not the baked-in wordmark, so the text stays crisp and
  themeable. The source crop coordinates and the two-tone lightness
  thresholds (background/dark-ink/light-ink) are worth preserving if this
  logo ever needs regenerating in a different accent color — see git
  history for the generation script.
