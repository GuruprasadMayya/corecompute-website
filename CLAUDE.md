...
- **Logo**: `assets/images/logo-mark.png` — a pixel-accurate cutout of the
  real corecompute.ae logo (not a hand-traced approximation), full lockup
  (icon + "CORE COMPUTE" wordmark), recolored two-tone blue/amber
  (`#3d8bff` / `#ffab40`) with a transparent background. Used directly as
  the `<img>` in every nav/footer `brand-mark` span — `.brand-name` is
  present in the markup for historical/accessibility reasons but is
  `display: none` in CSS (the image already contains the wordmark; an
  earlier pass paired an icon-only crop with separate HTML text, but that
  read as "the logo is missing its name" — the baked-in two-tone wordmark
  is the one users expect to see). `.brand-mark` sizes by `height` with
  `width: auto` to preserve the lockup's ~2.34:1 aspect ratio, not a fixed
  square.
  `logo-icon.png` (icon only, no wordmark, roughly square) still exists
  and is used *only* for the JSON-LD `logo` field — schema.org expects
  that field to be roughly square, and several platforms auto-crop it to
  a square avatar for previews; pointing `logo` at the wide lockup instead
  chopped off the wordmark when cropped (a real bug that shipped once —
  see git history). Don't swap `logo-mark.png`/`logo-icon.png` between the
  nav and the JSON-LD field, they solve two different problems.
  Both were generated by lightness-thresholding the original raster
  (background vs. ink, smoothed at the edges) rather than trying to
  unmatte per-pixel color — that threshold approach is robust to JPEG
  compression noise; an earlier per-pixel alpha/color attempt produced
  blotchy fringing artifacts and was discarded. The source crop
  coordinates and the two-tone lightness thresholds
  (background/dark-ink/light-ink) are worth preserving if this logo ever
  needs regenerating in a different accent color — see git history for
  the generation script.
  `assets/images/og-image.png` (1200×630) is a third derivative — the
  full lockup centered on a dark card, used for `og:image`/`twitter:image`
  so link previews in chat/social apps get a proper wide card instead of
  whatever a platform's auto-crop would produce from `logo-mark.png`.
  The real corecompute.ae logo has genuine dark-green/sage-green branding;
  this variant intentionally recolors it into the site's blue/amber accent
  pair rather than using the true brand colors, specifically so it doesn't
  clash with the dark blue theme. The beige variant uses the logo's true
  colors instead (see that variant's `CLAUDE.md`) — the two variants make
  different, deliberate choices here, don't "fix" one to match the other.
...
`assets/images/logo.png` (full raster, white background) and
`logo-source.jpeg` (original scrape) are still present but **unused** —
kept as the source material `logo-icon.png`/`logo-mark.png` were generated
from, in case they need regenerating with different crop/color parameters.
